...
def perform_clustering_analysis(df):
    ...
    for feature in df_cleaned.columns:
        ...
        if df_cleaned[feature].dtype in ['int64', 'float64']:
            feature_summary = df_cleaned.groupby('final_cluster')[feature].agg(['mean', 'std', 'min', 'max', 'count']).to_dict()
        else:
            feature_summary = df_cleaned.groupby('final_cluster')[feature].value_counts(normalize=True).unstack(fill_value=0).to_dict()
