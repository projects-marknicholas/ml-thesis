...
def generate_eda_visualizations(df, treatment_col, pre_features, outcome_col=None):
    ...
    try:
        ...
        # 2. Numeric Features Comparison (Box plots)
        ...
        if numeric_features:
            ...
            for i, feature in enumerate(top_numeric, 1):
                ...
                sns.boxplot(x=treatment_col, y=feature, data=df)
                plt.title(f'{feature} - Treatment vs Control')
        ...
        if outcome_col and outcome_col in df.columns:
            ...
            plt.subplot(1, 2, 1)
            sns.boxplot(x=treatment_col, y=outcome_col, data=df)
            plt.title(f'{outcome_col} - Treatment vs Control')
            plt.xlabel('Group (0=Control, 1=Treatment)')
            plt.ylabel(outcome_col)
            
            plt.subplot(1, 2, 2)
            outcome_means = df.groupby(treatment_col)[outcome_col].mean()
            plt.bar(['Control', 'Treatment'], outcome_means.values, color=['lightblue', 'lightcoral'])
            plt.title(f'Average {outcome_col} by Group')
            plt.ylabel(outcome_col)
        ...
        try:
            ...
            plt.title('Top 10 Features for Predicting Treatment Group')
        ...
        if missing_data.any().any():
            ...
            plt.title('Missing Values in Pre-Features')
            plt.xlabel('Features')
            plt.tight_layout()
...
def perform_clustering_analysis(df, selected_features=None):
    ...
    try:
        ...
        try:
            ...
            feature_weights.head(min(10, len(feature_weights))).plot(kind='barh')
            plt.gca().invert_yaxis()
            plt.title(f"Top Feature Weights - {best_model['Model']}")
            plt.xlabel("Weight")
            plt.tight_layout()
        ...
        try:
            ...
            plt.figure(figsize=(8, 6))
            scatter = sns.scatterplot(x=X_vis[:, 0], y=X_vis[:, 1], hue=final_labels, palette="Set2", s=60)
